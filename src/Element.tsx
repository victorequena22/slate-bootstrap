...
const Block = ({ attributes, children, element }: any) => {
  switch (element.type) {
    case 'heading-one':
      return (
        <h1 style={getCSS(element)} {...attributes}>
          {children}
        </h1>
      )
    case 'heading-two':
      return (
        <h2 style={getCSS(element)} {...attributes}>
          {children}
        </h2>
      )
    case 'heading-tre':
      return (
        <h3 style={getCSS(element)} {...attributes}>
          {children}
        </h3>
      )
    case 'heading-for':
      return (
        <h4 style={getCSS(element)} {...attributes}>
          {children}
        </h4>
      )
    default:
      return (
        <p style={getCSS(element)} {...attributes}>
          {children}
        </p>
      )
  }
...
const List = (p: any) => {
  ...
  return (
    <Block attributes={attributes} element={element}>
      {children}
    </Block>
  )
}
